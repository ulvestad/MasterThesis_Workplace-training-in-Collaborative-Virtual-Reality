...
\subsection{Changes}
The changes to the application is based on the analysis done in the previous phase. From table \ref{table:themesInterview1} we presented sub-themes which after some internal discussion resulted in an agreement (which we felt covered the most important aspects) that we should add the following changes to this phase's development and implementation. 

\begin{description}
    \item [Change 1]\hfill \\
    A fully functional virtual workplace environment must support multi-user functionality meaning objects, avatars, movement etc. should be networked and other data should be serialised in data stream to and from servers. 
    \item [Change 2]\hfill \\
    Direct communication so that users has the possibility to talk to each other. Using voice chat user should only hear others talk, not themselves, avoiding echo.  
    \item [Change 3]\hfill \\
    Collaborative features should be available for VR users, such as a laser pointer to pinpoint or mark objects and hand gesticulation.  
    \item [Change 4]\hfill \\
    More realistic avatars with some aspect of penalisation. 
\end{description}
